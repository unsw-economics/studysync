...
class ControlExperimentTest {
    ...
    private fun checkControlText() {
        // Check that the screen for control groups tells the user not to do anything
        delay()
        onView(withText(R.string.control_title)).check(matches(isDisplayed()))
        onView(withText(R.string.control_body)).check(matches(isDisplayed()))
    }
}